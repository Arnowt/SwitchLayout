# Auto-switch Layout Script (switch-layout.sh)
...
1. **Сохраните скрипт**
   Скачайте скрипт `switch-layout.sh` на ваш компьютер и сделайте его исполняемым:
   ```bash
   chmod +x /путь/к/скрипту/switch-layout.sh
   ```
...
3. **Укажите команду для запуска**
   ...
   * Пример команды:
   ```bash
   /путь/к/скрипту/switch-layout.sh "alt+shift"
   ```
